...
class CNN_LSTM_CTC_V2_CNN_more_filters_deeper_vgg16like(nn.Module):
    def __init__(self, img_height, num_channels, n_classes, n_h):
        """
        img_height: image height (after resize)
        num_channels: number of input channels (1 for grayscale)
        ...
        self.cnn = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.MaxPool2d((2, 1)), # downsample height by 8
            nn.Conv2d(144, 288, kernel_size=3, stride=1, padding=1),
            nn.ReLU(True),
        ...
        # After CNN, height becomes img_height // 8
        self.lstm_input_size = 288 * (img_height // 16)
...
class CNN_LSTM_CTC_V2_CNN_more_filters_batch_norm_more_imH(nn.Module):
    def __init__(self, img_height, num_channels, n_classes, n_h):
        """
        img_height: image height (after resize)
        num_channels: number of input channels (1 for grayscale)
        ...
        # After CNN, height becomes img_height // 8
        self.lstm_input_size = 256 * (img_height // 8)
