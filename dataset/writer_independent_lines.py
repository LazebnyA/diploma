...
def get_form_id_from_image_path(image_path):
    """
    Extracts form ID from line image filename (e.g. "a01-000u" from "a01-000u-00.png").
    """
...
def writer_independent_split(line_mappings_file, metadata_folder, output_dir, ratios=(0.7, 0.15, 0.15)):
    """
    Creates writer-independent train, validation, and test splits for line images.

    Args:
        line_mappings_file: Path to the file mapping line image paths to text
    ...
    # Read the line mappings file
    entries = []  # list of tuples: (image_path, text, writer_id)
    with open(line_mappings_file, 'r', encoding='utf-8') as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            ...
            if writer_id is None:
                ...
                continue
            entries.append((image_path, text, writer_id))
    ...
    # Output file paths
    train_output = os.path.join(output_dir, "train_lines_mappings.txt")
    val_output = os.path.join(output_dir, "val_lines_mappings.txt")
    test_output = os.path.join(output_dir, "test_lines_mappings.txt")

    # Write out the new line mapping files
    with open(train_output, 'w', encoding='utf-8') as f_train:
        for img_path, text, _ in train_entries:
            f_train.write(f"{img_path}\t{text}\n")

    with open(val_output, 'w', encoding='utf-8') as f_val:
        for img_path, text, _ in val_entries:
            f_val.write(f"{img_path}\t{text}\n")

    with open(test_output, 'w', encoding='utf-8') as f_test:
        for img_path, text, _ in test_entries:
            f_test.write(f"{img_path}\t{text}\n")
    ...
    with open(os.path.join(output_dir, "lines_split_summary.txt"), 'w', encoding='utf-8') as f_summary:
        ...
        # Extra statistics about line counts
        f_summary.write("\nText length statistics:\n")
        train_chars = sum(len(text) for _, text, _ in train_entries)
        val_chars = sum(len(text) for _, text, _ in val_entries)
        test_chars = sum(len(text) for _, text, _ in test_entries)
        f_summary.write(f"Train: {train_chars} characters ({train_chars / len(train_entries):.1f} avg per line)\n")
        f_summary.write(f"Validation: {val_chars} characters ({val_chars / len(val_entries):.1f} avg per line)\n")
        f_summary.write(f"Test: {test_chars} characters ({test_chars / len(test_entries):.1f} avg per line)\n")


if __name__ == "__main__":
    # Specify your paths here
    LINE_MAPPINGS_FILE = "line_mappings.txt"
    METADATA_FOLDER = "metadata"
    OUTPUT_DIR = "writer_independent_line_splits"

    # Default ratio: 70% train, 15% validation, 15% test
    writer_independent_split(LINE_MAPPINGS_FILE, METADATA_FOLDER, OUTPUT_DIR)